...
def train():
    ...
    print("tfds.load......", flush=True)
    examples, metadata = tfds.load('ted_hrlr_translate/pt_to_en', with_info=True, as_supervised=True)
    print("tfds.load......done", flush=True)
    ...
    print("tokenizer gen done",flush=True)
    ...
    def filter_long_sent(x, y, max_length=40):
    ...
    transformer = Transformer(num_layers, d_model, num_heads, dff, input_vocab_size, target_vocab_size, max_seq_len, dropout_rate)
    ...
    for epoch in range(EPOCHS):
        start = time.time()
        print("train....epoch={}".format(epoch), flush=True)
        ...
        for batch, (inputs, targets) in enumerate(train_dataset):
            ...
            if batch % 500 == 0:
                print('epoch {}, batch {}, loss:{:.4f}, acc:{:.4f}'.format(epoch + 1, batch, train_loss.result(), train_accuracy.result()))
